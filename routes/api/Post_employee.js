...
//bring post model 
const Posts = require('../../models/Post_employee');
...
//explain   create post 
//access    role private
router.post('/',Passport.authenticate('jwt-1',{ session: false }),(req,res)=>{
...
//explain   show  specific post 
//access    role private
router.delete('/show/:id',Passport.authenticate('jwt-1',{ session: false }),(req,res)=>{
...
//route     POST api/employee/post/show/like/:id
//explain   like specific post
//access    role private
router.post('/show/like/:id',Passport.authenticate('jwt-1',{ session: false }),(req,res)=>{
    Posts.findOne({user: req.user.id})
    .then(post =>{
        Posts.findById(req.params.id)
        .then(post => {
          if(post.likes.filter(like =>
              like.user.toString() ===  req.user.id).length > 0){
                  res.status(400).json({liked: "employee alreday liked this post"})
              }else{
...
//route     POST api/employee/post/show/dislike/:id
//explain   dislike specific post
//access    role private
router.post('/show/dislike/:id',Passport.authenticate('jwt-1',{ session: false }),(req,res)=>{
    Posts.findOne({user: req.user.id})
    .then(post =>{
        Posts.findById(req.params.id)
        .then(post => {
...
//route     POST api/employee/post/show/comment/:id
//explain   comment on a specific post
//access    role private
router.post('/show/comment/:id',Passport.authenticate('jwt-1',{ session: false }),(req,res)=>{
    const  {error,isValid} = validatePost(req.body)
    if(!isValid){
         res.status(400).json(error)
    }else{
...
//route     DELETE api/employee/post/show/comment/:id
//explain   delete a  comment
//access    role private
router.delete('/show/comment/:id/:comment',Passport.authenticate('jwt-1',{ session: false }),(req,res)=>{
